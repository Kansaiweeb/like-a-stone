...
impl MapBuilder for BspDungeonBuilder {
    ...
    fn spawn_entities(&mut self, _commands: &mut Commands) {
...
impl BspDungeonBuilder {
    ...
    fn build(&mut self) {
        ...
        // Up to 240 times, we get a random rect and divide it.
        // If its possible to squeeze a room in there, we place it
        // and it to the rooms list.
        let mut n_rooms = 0;
        while n_rooms < 240 {
    ...
    fn get_random_sub_rect(&self, rect: Rect, rng: &mut RandomNumberGenerator) -> Rect {
        let mut result = rect;
        let rect_width = i32::abs(result.x1 - result.x2);
        let rect_height = i32::abs(result.y1 - result.y2);
